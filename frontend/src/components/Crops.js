    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="section-shell">
      <div className="section-hero">
        <div>
          ...
          <button
            onClick={() => { setShowAdd(true); setEditData(null); }}
          ...
          {fields.map((field) => (
            <motion.div key={field._id} whileHover={{ scale: 1.03 }} className="panel-card p-6">
              <div className="flex justify-between items-start">
                ...
                <button
                  onClick={() => { setEditData(field); setShowAdd(true); }}
                  className="text-sm text-green-700 hover:text-green-900 border border-green-200 rounded px-3 py-1"
                >
                  Edit
                </button>
              </div>
